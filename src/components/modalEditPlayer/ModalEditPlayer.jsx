...
const ModalEditPlayer = () => {
  ...
  return (
    <div className='grid grid-cols-12 gap-0 text-white animate__animated animate__bounceInDown'>
      <div className={`col-start-2 col-span-10 md:col-start-2 md:col-span-10 lg:col-start-4 lg:col-span-6 ${style.modalEdit}`}>
        <div className='flex justify-between'>
          <p className='text-2xl md:text-3xl py-2'>{modalOn.player===1?'Jugador 1':'Jugador 2'}</p>
          <button className={`text-2xl px-4 py-2 ${style.buttExit}`} onClick={()=>{setModal({on:false,player:null})}}>X</button>
        </div>
        <div className='mt-3'>
          <form onSubmit={handleSubmit}>
            <div className='flex flex-col items-center mb-4'>
              <p className='text-lg md:text-xl mb-3 text-center'>Editar Nombre</p>
              <input type='text' id='InputName' placeholder='Ingresar nombre' maxLength={10} className={`w-4/5 md:w-2/4 ${style.inputname}`} onChange={handleChange}></input>
            </div>
            <div className='flex flex-col items-center'>
              <p className='text-lg md:text-xl mb-3 text-center'>Elegir personaje</p>
              <div className='grid grid-cols-12 gap-1'>
                <button 
                  className={`col-start-1 col-span-4 md:col-start-4 md:col-span-2 ${click===0 ? style.activeChar : ''}`} onClick={()=>{setClick(0); setSelectChar(0)}} type='button'>
                ...
                <button type='button' className={`col-span-4 md:col-span-2 ${click===1 ? style.activeChar: ''}`} onClick={()=>{setClick(1);setSelectChar(1)}}>
                  <div className={`${style.character}`}>
                    <img src={charsIcons.char1Icon} alt="" />
                  </div>
                </button>
                <button type='button' className={`col-span-4 md:col-span-2 ${click===2 ? style.activeChar:''}`} onClick={()=>{setClick(2);setSelectChar(2)}}>
                  <div className={`${style.character}`}>
                    <img src={charsIcons.char2Icon} alt="" />
                  </div>
                </button>
                <button type='button' className={`col-start-1 col-span-4 md:col-start-4 md:col-span-2 ${click===3 ? style.activeChar:''}`} onClick={()=>{setClick(3);setSelectChar(3)}}>
                  <div className={`${style.character}`}>
                    <img src={charsIcons.char3Icon} alt="" />
                  </div>
                </button>
                <button type='button' className={`col-span-4 md:col-span-2 ${click===4 ? style.activeChar:''}`} onClick={()=>{setClick(4);setSelectChar(4)}}>
                  <div className={`${style.character}`}>
                    <img src={charsIcons.char4Icon} alt="" />
                  </div>
                </button>
                <button type='button' className={`col-span-4 md:col-span-2 ${click===5 ? style.activeChar:''}`} onClick={()=>{setClick(5);setSelectChar(5)}}>
                  <div className={`${style.character}`}>
